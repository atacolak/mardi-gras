...
Mardi Gras is a terminal UI (TUI) that visualizes [Beads](https://github.com/gastownhall/beads) issues as a priority-sorted work tree. The tree keeps work in one hierarchy, with Waiting/Blocked and Deferred rendered as attention sections only when they contain rows; it does not turn the six semantic states into static columns.
...
internal/
  ...
  data/
    ...
    semantic.go           Six semantic execution states (including OperatorReview), DeriveState, grouping
  ...
  views/
    parade.go             Left pane: priority-sorted work tree plus two attention cuts
...
**`views.Parade`** — Maintains a flat `[]ParadeItem` render cache, a cursor position, scroll offset, and per-ID `Collapsed` set. It builds one parent-first forest for Ready, Working, OperatorReview, and Done; Done remains nested under its owning epic. Waiting/Blocked and Deferred are the only attention sections, each rendered only when non-empty. Navigation methods (`MoveUp`, `MoveDown`) skip non-selectable headers and footers. Supports multi-select (`selectedIDs` set) for bulk operations.

The parade is tree-first: ancestry comes from loaded `parent-child` edges, while roots and siblings use priority then stable ID. Collapsing a node hides only its descendants and preserves header counts; there is no global Done fold or recency ordering claim.
...
The persisted semantic order is `Ready`, `Working`, `Waiting/Blocked`, `Deferred`, `OperatorReview`, `Done`. A raw `review` status is authoritative and derives `OperatorReview` before dependency blocking; the lead writes that gate explicitly with `br update <epic> --status review`, and `review` is not part of Ready. The compatibility shim for already-converged legacy epics is separate from this stored-state path.
